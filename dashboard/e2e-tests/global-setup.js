...
export default async function globalSetup() {
  ...
  try {
    ...
    console.log('📱 Checking frontend health at http://localhost:5173...');
    await page.goto('http://localhost:5173', { waitUntil: 'networkidle' });
    console.log('✅ Frontend is ready');

    // Wait for backend to be ready
    console.log('🔧 Checking backend health at http://localhost:8000...');
    ...
    const statusResponse = await page.request.get('http://localhost:8000/v1/virtuous-cycle/status');
    if (statusResponse.ok()) {
      console.log('✅ Virtuous Cycle API is ready');
    } else {
      throw new Error(`Virtuous Cycle API check failed: ${statusResponse.status()}`);
    }
