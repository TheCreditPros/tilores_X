...
class TestVirtuousCycleAPIEndpoints:
    ...
    async def test_virtuous_cycle_trigger_endpoint_success(self):
        ...
        with patch('main_enhanced.virtuous_cycle_manager') as mock_manager:
            mock_manager.trigger_manual_optimization = AsyncMock(
                return_value=self.mock_trigger_success
            )
            ...
            mock_manager.trigger_manual_optimization.assert_called_once_with(
                trigger_data['reason']
            )
    ...
    async def test_virtuous_cycle_trigger_endpoint_cooldown(self):
        ...
        with patch('main_enhanced.virtuous_cycle_manager') as mock_manager:
            mock_manager.trigger_manual_optimization = AsyncMock(
                return_value=cooldown_response
            )

            response = self.client.post(
                "/v1/virtuous-cycle/trigger",
                json={"reason": "Test during cooldown"}
            )
    ...
    async def test_virtuous_cycle_trigger_endpoint_error(self):
        ...
        with patch('main_enhanced.virtuous_cycle_manager') as mock_manager:
            ...
            response = self.client.post(
                "/v1/virtuous-cycle/trigger",
                json={"reason": "Test failure scenario"}
            )
