...
class TestVirtuousCycleSimple:
    ...
    async def test_trigger_manual_optimization_success(self):
        ...
        result = await self.manager.trigger_manual_optimization("Test trigger")
    ...
    async def test_trigger_manual_optimization_cooldown(self):
        ...
        result = await self.manager.trigger_manual_optimization("Test during cooldown")
    ...
    async def test_start_monitoring_method(self):
        ...
        with patch.object(self.manager, "_trace_monitoring_loop") as mock_trace, patch.object(
            self.manager, "_quality_monitoring_loop"
        ) as mock_quality, patch.object(self.manager, "_optimization_loop") as mock_opt, patch.object(
            self.manager, "_trace_processor"
        ) as mock_processor:
    ...
    async def test_analyze_trace_batch_with_data(self):
        ...
        mock_traces = [
            {"quality_score": 0.95, "spectrum": "customer_profile", "model": "gpt-4o-mini"},
            {"quality_score": 0.88, "spectrum": "credit_analysis", "model": "claude-3-haiku"},
        ]
