...
class AIPromptOptimizer:
    ...
    def __init__(self, langsmith_api_key: Optional[str] = None):
        ...
        self.langsmith_api_key = langsmith_api_key or os.getenv(
            'LANGSMITH_API_KEY'
        )
        self.client = None
    ...
    async def create_optimization_cycle(
        ...
        optimizations = await self.generate_spectrum_optimizations(
            analyses, baseline_results
        )
    ...
    async def save_optimization_results(
        ...
        results["summary"] = {
            ...
            "average_expected_improvement": statistics.mean([
                opt.expected_improvement for opt in cycle.optimizations
            ]),
            "highest_potential_spectrum": max(
                cycle.optimizations,
                key=lambda x: x.expected_improvement
            ).spectrum,
            "lowest_performing_spectrum": min(
                cycle.optimizations,
...
async def main():
    ...
    try:
        ...
        print(f"   🎯 Target Achievement: {(statistics.mean(cycle.baseline_scores.values()) + cycle.overall_improvement_potential):.1%}")  # noqa: E501
