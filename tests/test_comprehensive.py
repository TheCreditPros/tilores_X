...
from test_data import (
    TEST_CUSTOMERS, PHONE_SCENARIOS, BATCH_SCENARIOS,
    COMPLEX_SCENARIOS, EDGE_CASES, PERFORMANCE_TARGETS,
    get_test_customer, get_customers_for_prewarm, get_batch_test_data
)
...
class ComprehensiveTestSuite:
    ...
    def test_batch_processing(self):
        ...
        try:
            ...
            if passed:
                print(f"✅ Batch processing: {speedup:.1f}x speedup (target: {PERFORMANCE_TARGETS['batch_speedup_factor']}x)")
            else:
                print(f"⚠️  Batch processing: {speedup:.1f}x speedup (target: {PERFORMANCE_TARGETS['batch_speedup_factor']}x)")
